...
export function credentialsFileExists(filePath) {
...
export function formatCredentialsText(values) {
  const lines = [
    "# Generated by refrens-api setup.",
    `app_id=${JSON.stringify(values.app_id ?? "")}`,
    `app_secret=${JSON.stringify(values.app_secret ?? "")}`,
    `url_key=${JSON.stringify(values.url_key ?? "")}`,
    `base_url=${JSON.stringify(values.base_url ?? "")}`
  ];

  if (typeof values.private_key === "string" && values.private_key.trim()) {
    lines.push(`private_key=${JSON.stringify(values.private_key)}`);
  }
...
export function writeCredentialsFile(
  ...
  if (fs.existsSync(filePath) && !overwrite) {
    throw new Error(`Credentials file already exists: ${filePath}`);
  }

  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, formatCredentialsText(values), "utf8");
}
